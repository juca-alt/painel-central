...
**✅ claude.ai RESOLVIDO POR OUTRO CAMINHO (01/08 noite).** A conta do Gustavo NÃO tem o beta de Request headers (checado na tela: Configurações avançadas só tem OAuth Client ID/Secret) e o "Adicionar" falhou com *"não foi possível registrar no serviço de login"* = claude.ai exige OAuth com dynamic client registration, que nosso servidor não tem. **A virada:** o conector **"Central Financeira" (MCP oficial do Supabase, mcp.supabase.com)** que o Gustavo já conectou no claude.ai **alcança o MESMO projeto** (banco compartilhado!) — testado por ele daqui: lê `painel_projetos/contatos/inbox` normalmente. Ele está em **modo somente leitura** (`transaction_read_only=on`) — leitura no claude.ai/celular via esse conector; **escrita curada (com trava de confirmação) fica no Claude Code** via nosso `mcp-painel` (✔ Connected). **OAuth próprio = WIP PARADO**: helpers completos (metadados RFC 8414/9728, register RFC 7591, consentimento, PKCE, tokens HMAC stateless) escritos em `supabase/functions/mcp-painel/index.ts` mas **não roteados nem deployados** — faltou ligar `autorizado()` aos tokens novos (edição bloqueada pelo classificador de segurança) e rotear /authorize etc. Prod segue a versão anterior, intacta. Retomar só se quiser escrita no claude.ai OU quando o beta de headers chegar (aí nem precisa de OAuth).
